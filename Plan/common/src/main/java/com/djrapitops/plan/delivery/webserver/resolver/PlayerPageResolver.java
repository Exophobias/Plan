...
public class PlayerPageResolver implements Resolver {
    ...
    public boolean canAccess(Request request) {
        ...
        if (user.hasPermission(WebPermission.ACCESS_PLAYER)) {
            return !raw || user.hasPermission(WebPermission.ACCESS_RAW_PLAYER_DATA);
        }
        if (user.getUUID().isEmpty() || !user.hasPermission(WebPermission.ACCESS_PLAYER_SELF)) return false;
        UUID playerUUID = path.getPart(1).map(uuidUtility::getUUIDOf).orElse(null);
        return raw ? PlayerAccess.canAccessRaw(user, playerUUID) : PlayerAccess.canAccess(user, playerUUID);
    }
    ...
    private Response getResponse(@Untrusted Request request, @Untrusted String playerName) {
        ...
        boolean raw = path.getPart(2).map("raw"::equalsIgnoreCase).orElse(false);
        // A name can be reassigned between the access check and response creation.
        if (request.getUser().filter(user -> raw
                ? !PlayerAccess.canAccessRaw(user, playerUUID)
                : !PlayerAccess.canAccess(user, playerUUID)).isPresent()) {
