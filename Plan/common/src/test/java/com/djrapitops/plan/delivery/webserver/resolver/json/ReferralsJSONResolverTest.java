...
import com.djrapitops.plan.referrals.ReferralAnalyticsSvc;
import org.junit.jupiter.api.Test;
import java.util.*;
import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.Mockito.*;

class ReferralsJSONResolverTest {
    private final WebServer web = mock(WebServer.class);
    private final Identifiers identifiers = mock(Identifiers.class);
    private final ReferralAnalyticsSvc service = mock(ReferralAnalyticsSvc.class);
    private final ReferralsJSONResolver resolver = new ReferralsJSONResolver(identifiers,service,() -> web);
    private Request request(String... grants) {
        return new Request("GET","/v1/referrals?server=1",new WebUser("staff",UUID.randomUUID(),"staff",List.of(grants)),Map.of());
    }
    @Test void publicModeRejectsEvenExplicitPermissionsWhenResolveCalledDirectly() {
        when(web.isAuthRequired()).thenReturn(false);
        assertEquals(403,resolver.resolve(request("page.server.referrals","access.server")).orElseThrow().getCode());
    ...
    @Test void anonymousSelfAndParentPagePermissionsDoNotGrantReferralAccess() {
        ...
        for (Request req : List.of(request("page","access.server"),request("page.server","access.server"),request("page.player","access.player.self"),
                new Request("GET","/v1/referrals?server=1",null,Map.of()))) {
    ...
    @Test void explicitReferralPermissionStillRequiresServerAccess() {
        when(web.isAuthRequired()).thenReturn(true);
        assertEquals(403,resolver.resolve(request("page.server.referrals")).orElseThrow().getCode());
    }
    @Test void authenticatedResponseKeepsNullsPrivateAndCorrectServerScope() {
        ...
        when(service.report(server)).thenReturn(output);
        var response = resolver.resolve(request("page.server.referrals","access.server")).orElseThrow();
        ...
        verify(service).report(server);
    }
    @Test void unknownServerCannotFallBackToAnotherServersCachedData() {
        when(web.isAuthRequired()).thenReturn(true); UUID server = UUID.randomUUID();
        when(identifiers.getServerUUID(any(Request.class))).thenReturn(ServerUUID.from(server));
        when(service.report(server)).thenThrow(new IllegalArgumentException());
        assertEquals(400,resolver.resolve(request("page.server.referrals","access.server")).orElseThrow().getCode());
    }
    @Test void collectionFailureDoesNotReturnConvincingZeroMeasurements() {
        when(web.isAuthRequired()).thenReturn(true); UUID server = UUID.randomUUID();
        when(identifiers.getServerUUID(any(Request.class))).thenReturn(ServerUUID.from(server));
        when(service.report(server)).thenThrow(new IllegalStateException());
        assertEquals(503,resolver.resolve(request("page.server.referrals","access.server")).orElseThrow().getCode());
    }
}
